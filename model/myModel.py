...
class CnnLSTM(nn.Module):
    def __init__(self,embedding_dim,hidden_dim,layer_dim,out_channels):
        # [8,441,2020] #[8,44100]
        # vocab_size:词典长度 # embedding_dim:词向量维度 44100
        # hidden_dim:神经元个数 # layer_dim:网络层数
        # output_dim:隐藏层输出的维度（分类的数量）
        super(CnnLSTM, self).__init__()
        self.lstm = nn.LSTM(embedding_dim,hidden_dim,layer_dim,batch_first=True,bidirectional=False)
        # 一维卷积神经网络
        self.conv1 = nn.Conv1d(1, out_channels=out_channels, kernel_size=64,stride=2)
        self.BN1 = BatchNormalization()
        self.max_pool1 = nn.MaxPool1d(4)
        self.conv2 = nn.Conv1d(1, out_channels=32, kernel_size=32, stride=2)
        self.BN2 = BatchNormalization()
        self.max_pool2 = nn.MaxPool1d(4)
        self.conv3 = nn.Conv1d(1, out_channels=64, kernel_size=16, stride=2)
        self.BN3 = BatchNormalization()
        self.max_pool3 = nn.MaxPool1d(4)
        self.conv4 = nn.Conv1d(1, out_channels=128, kernel_size=8, stride=2)
        self.BN4 = BatchNormalization()
        self.max_pool4 = nn.MaxPool1d(4)
        self.conv5 = nn.Conv1d(1, out_channels=256, kernel_size=4, stride=2)
        self.BN5 = BatchNormalization()
        self.max_pool5 = nn.MaxPool1d(4)
        self.fc1 = nn.Linear(out_channels * 625,  120)
        self.fc2 = nn.Linear(120, 84)
        self.fc3 = nn.Linear(84, 5)
    def forward(self, x):
        x, _ = self.lstm(x)
        x = self.conv(x)
        x = self.max_pool(F.tanh(x))
        x = x.view(-1, 64 * 625)
        x = F.tanh(self.fc1(x))
        x = F.tanh(self.fc2(x))
        x = self.fc3(x)
